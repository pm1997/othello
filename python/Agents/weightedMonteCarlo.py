...
class WeightedMonteCarlo:
    ...
    def __init__(self, big_number=10, use_start_libs=True, preprocessor_n=-1, heuristic=heuristics.StoredMonteCarloHeuristic.heuristic, use_multiprocessing=True):
        """
        ...
        if preprocessor_n == 0:
            ...
            self._preprocessor = WeightedMonteCarlo.preprocess_variable_selectivity
    ...
    def select_preprocessor():
        ...
        available_preprocessors.append(
            ("Fixed Selectivity Preprocessor (FSP)", WeightedMonteCarlo.preprocess_fixed_selectivity))
        available_preprocessors.append(
            ("Variable Selectivity Preprocessor (VSP)", WeightedMonteCarlo.preprocess_variable_selectivity))
        ...
        if preprocessor == WeightedMonteCarlo.preprocess_fixed_selectivity:
            preprocessor_parameter = UtilMethods.get_integer_selection(
                ...
                1, 64)
        elif preprocessor == WeightedMonteCarlo.preprocess_variable_selectivity:
    ...
    def preprocess_fixed_selectivity(game_state: Othello, n_s, heuristic):
        ...
        heuristic_values = sorted(
            WeightedMonteCarlo.preprocess_get_heuristic_value(game_state, heuristic=heuristic).items(),
    ...
    def preprocess_variable_selectivity(game_state: Othello, p_s, heuristic):
        ...
        heuristic_value_dict = WeightedMonteCarlo.preprocess_get_heuristic_value(game_state, heuristic=heuristic)
    ...
    def get_weighted_random(possible_moves, turn_nr, own_symbol):
        ...
        for move in possible_moves:
            prob_sum += WeightedMonteCarlo._ml_database.get_likelihood(move, turn_nr, own_symbol)
    ...
    def play_weighted_random_game(own_symbol, simulated_game):
        ...
        first_move = WeightedMonteCarlo.get_weighted_random(possible_moves, simulated_game.get_turn_nr(), own_symbol)
        # play move
        simulated_game.play_position(first_move)
        # play whole remaining game
        while not simulated_game.game_is_over():
            possible_moves2 = simulated_game.get_available_moves()
            move = WeightedMonteCarlo.get_weighted_random(possible_moves2, simulated_game.get_turn_nr(), own_symbol)
            simulated_game.play_position(move)
        won = 1 if simulated_game.get_winner() == own_symbol else 0
    ...
    def play_n_random_games(own_symbol, game_state: Othello, number_of_games):
        ...
        for _ in range(number_of_games):
            ...
            first_played_move, won = WeightedMonteCarlo.play_weighted_random_game(own_symbol, simulated_game)

#            first_played_move, won = WeightedMonteCarlo.play_random_game(own_symbol, simulated_game)
    ...
    def get_move(self, game_state: Othello):
        ...
        if not self._use_multiprocessing:
            winning_statistics = WeightedMonteCarlo.play_n_random_games(own_symbol, game_state, self._big_n)
        else:
            number_of_processes = mp.cpu_count()
            pool = mp.Pool(processes=number_of_processes)
            list_of_stats = [pool.apply_async(WeightedMonteCarlo.play_n_random_games, args=(own_symbol, game_state.deepcopy(), self._big_n // number_of_processes)) for _ in range(number_of_processes)]
            winning_statistics = list_of_stats[0].get()
            for single_list in list_of_stats[1:]:
                WeightedMonteCarlo.combine_statistic_dicts(winning_statistics, single_list.get())
            pool.close()
