...
\begin{lstlisting}[basicstyle=\footnotesize,caption = {get\_move Funktion des Monte-Carlo Agenten}, language = python, captionpos = t , numbers=left, label={lst:ab1}]
...
%
%\begin{lstlisting}[caption = {get\_weighted\_random Funktion des Monte-Carlo Agenten}, language = python, captionpos = t , numbers=left, label={lst:wmc1}]
...
\\Die Übergabeparameter, sowie die Zeilen bis einschließlich 5 \improvement{anders} sind identisch zu der Implementierung der im Kapitel \ref{mc-getmove} diskutierten \code{get\_move}-Funktion des \mxZitat{Monte-Carlo}-Agenten.
...
\begin{lstlisting}[basicstyle=\footnotesize,caption = {get\_move Funktion des Alpha-Beta Spielers}, language = python, captionpos = t , numbers=left, label={lst:ab-get-move}]
    def get_move(self, game_state: Othello):
