...
Die \mxZitat{Stored Monte-Carlo-Heuristik} verwendet eine Datenbank. In dieser Datenbank werden die Anzahl der gespielten und gewonnenen Spielzüge gespeichert. Diese wird in den Unterkapiteln \ref{db}  und \ref{train} erklärt. Darauf aufbauend wird die Funktionsweise der Heuristik in dem Kapitel \ref{heuristik1} erläutert.
\paragraph{Datenbank}
\label{db}
...
\\Diese Züge werden im Spiel mit dem aktuellen Spielzustand verglichen. Wenn eine oder mehrere gespeicherte Züge mit dem aktuellen Spielzustand übereinstimmen, wir ein Zug aus den verfügbaren Zügen ausgewählt und gespielt. Die Funktion \code{get\_available\_start\_tables} ist in Listing \ref{lst:start2} abgebildet.
...
\end{itemize}
\item \code{preprocessor\_parameter}: Parameter des verwendeten Preprozessors. Je nach Auswahl des Präprozessors steht dieser Parameter für:
\begin{itemize}
...
\end{itemize}
\item \code{heuristic}: Eine der in \ref{heuristik1} beschriebenen. Wird im Präprozessor zur Bewertung von Spielzuständen herangezogen.
...
\\Die Spielerklasse \mxZitat{alphaBetaPruning} ist in der Datei \mxZitat{python/Agents/alphaBetaPruning.py} zu finden.
...
In dem Konstruktor der Klasse \code{AlphaBetaPruning} können folgende Klassenvariablen gesetzt werden:
\begin{itemize}
\item \code{\_search\_depth}: Tiefe der Alpha-Beta Suche
\item \code{\_use\_start\_libs}: Bool'scher Wert ob Startbibliotheken verwendet werden sollen.
\item \code{\_heuristic}: Nummer der verwendeten Heuristik
\item \code{\_use\_ml}: Verwende statt der Heuristik Machine Learning in der Tiefe \code{search\_depth} + 1
\item \code{\_use\_monte\_carlo}: Verwende statt der Heuristik Monte Carlo in der Tiefe \code{search\_depth} + 1
\item \code{\_ml\_count}: Anzahl der zufällig gespielten Spiele wenn Machine Learning oder Monte Carlo verwendet wird.
\end{itemize}
