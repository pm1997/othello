...
\\Die mathematischen Formeln dazu lauten:
\\$\mathtt{Categories}: \{ 0,...,8\} $: Menge der definierten Feldkategorien
\\$\mathtt{category}: \mathtt{Moves} \rightarrow \mathtt{Categories}$: Gibt die Feldkategorie des Spielfeldes zurück.
\\$\mathtt{database}: \mathbb{N} \times \mathtt{Categories} \rightarrow < \mathbb{N} ,\mathbb{N} ,\mathbb{N}>$: Tripel bestehend aus Anzahl gewonnener Spiele des ersten, bzw. zweiten Spielers und Gesamtanzahl der Spiele
\\$\mathtt{w}: < \mathbb{N} ,\mathbb{N} ,\mathbb{N}> \times\ \mathtt{Players} \rightarrow \mathbb{N}$ : Anzahl der Gewinne des übergebenen Spielers
\\$\mathtt{t}: < \mathbb{N} ,\mathbb{N} ,\mathbb{N}> \times\ \mathtt{Players} \rightarrow \mathbb{N}$ : Gesamtanzahl der durchgeführten Spiele
\\$\mathtt{turn}: \mathtt{Q} \rightarrow \mathbb{N} $ : Zugnummer des Zustandes Q
\\Sei $s \in \mathtt{Q}, m \in \mathtt{actions}(s)$
\\dann gilt:
\\{\large
$P( s, m) = \frac{w(\mathtt{database}(\mathtt{turn}(s),\mathtt{category}(m)),\mathtt{players}(s))}{\mathtt{t}(\mathtt{turn}(s),\mathtt{category}(m))} $
}
...
Die Startwerte der Tupel sind jeweils \mxZitat{(0, 0, 0)}, da initial noch keine Spiele gespeichert sind. Diese werden in der Membervariable \code{\_db\_data} gespeichert.
