...
\\$\mathtt{MiniMax}(s,p) = \begin{cases} \mathtt{Utility}(s,p)$; wenn TerminalTest(s) $\\\mathtt{max}(\{\mathtt{MiniMax}(\mathtt{result}(s,a) | a \in \mathtt{actions}(s)\},(p +1)\%2 )$; wenn Spieler am Zug$\\\mathtt{min}(\{\mathtt{MiniMax}(\mathtt{result}(s,a) | a \in \mathtt{actions}(s) \}, (p +1)\%2)$; wenn Gegner am Zug$\end{cases}$
...
\\$\mathtt{getBestMove}(s,\mathtt{score}, p) =  \{ a | a \in \mathtt{actions} \wedge \mathtt{MiniMax}(\mathtt{result}(s,a), (p + 1) \% 2 ) == \mathtt{score}\}.\mathtt{any}()$
