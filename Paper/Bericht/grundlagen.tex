...
\item $\mathtt{player}: \mathtt{Q} \rightarrow p \in \mathtt{Players}$ ist auf der Menge der Spieler definiert und gibt den aktuellen Spieler p zurück.
\item $\mathtt{actions}: \mathtt{Q} \rightarrow  \mathtt{moves}$ gibt die validen Zugmöglichkeiten (moves) eines gegeben Zustands zurück.
...
In einem \blue{Nullsummenspiel} ist die Summe der utility Funktion eines Endzustandes ($\mathtt{terminalState}$) über alle Spieler 0 ($\sum\limits_{p \in Players} utility(p) = 0$).
\\Dies bedeutet, dass wenn ein Spieler gewinnt mindestens ein Gegenspieler verliert.
\\In Othello spielen zwei Spieler gegeneinander. Es gibt als nur die drei Möglichkeiten:
\begin{itemize}
...
\end{itemize}
\end{Definition}
Durch den Startzustand $\so$ und der Funktion $\mathtt{actions}$ wird ein \blue{Spielbaum (Game Tree)} aufgespannt.
...
\\$A = [s,\{\}] \in \mathtt{GameTree}$ : A, bestehend aus einem Zustand, ist ein Spielbaum.
\\$B = [s,\{A\}] \in \mathtt{GameTree}$ : B, bestehend aus einem Zustand s und einem Kindknoten, ist ein Spielbaum.
\\$C = [s,T] \in \mathtt{GameTree}$, wobei T eine Menge von Spielbäumen ist: C ist ein Spielbaum.
\end{Definition}
...
\\$MiniMax = \begin{cases} Q \rightarrow \{-1,0,1\}$; wenn $\mathtt{Q} \in \mathtt{TerminalStates} \\ Q \rightarrow \mathtt{actions}$ sonst $\end{cases}$
...
Nachfolgend wird eine Pseudoimplementierung einer Invariante des Alpha-Beta Abschneiden Algorithmus angegeben (siehe Listing \ref{lst:abprun}). Es handelt sich um eine angepasste Version von \info{Quelle Stroetmann}:
