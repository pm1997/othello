...
Nachfolgend wird eine Pseudoimplementierung des um Alpha-Beta Pruning erweiterten MiniMax Algorithmus angegeben (siehe Listing \ref{lst:abprun}):
\begin{footnotesize}
...
global Suchtiefe
int miniMax(State AktuellerZustand, int Spieler, int Tiefe, int alpha, int beta) {
	if (Tiefe == 0) {
	...
	for (Zug z in Zuege) {
		...
		wert = -miniMax(NeuerZustand, anderer(Spieler), Tiefe-1, -beta, -bisherigerMaximalWert)
