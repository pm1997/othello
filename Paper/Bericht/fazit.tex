...
\\Auffällig ist außerdem, dass im Vergleich zu dem \mxZitat{\mc}-Agenten im Spiel mit dem \mxZitat{Random}-Agenten eine deutlich längere Rechenzeit benötigt wird um zu einem insgesamt schlechteren Ergebnis zu kommen. So gelang es dem Agenten \mxZitat{\mc} alle Spiele gegen den Agenten \mxZitat{Random} zu gewinnen. {\color{red} Der hier besprochene Agent gewinnt jedoch, selbst in seiner besten Variation, nur $90\%$ der Spiele.} Auf die Performance der beiden Agenten im direkten Vergleich wird in Abschnitt \ref{subsec:Fazit:AgentenVgl} eingegangen.
\paragraph{Weitere Variationsmöglichkeiten}
...
Der \mxZitat{\mc} Agent spielt sowohl bei der Verwendung der \mxZitat{Cowthello}-Heuristik {\color{red}als auch bei der Verwendung der \mc -Variante} deutlich besser als der \mxZitat{\abp} Agent. Dieses Ergebnis kann dabei sogar bei einer im Vergleich nicht wesentlich längeren Rechenzeit erreicht werden.
